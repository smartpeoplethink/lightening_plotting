TIME_FRAME1 = ["00:55:34.4","00:55:35.1"]
TIME_FRAME2 = ["00:57:50.8", "00:57:52.2"]
...
Gradient_Pink = True
NonSLname = "DATA"
graphName = "Lightning v1"

csv_file = r"C:\Users\Samuel Halperin\OneDrive\Documents\GitHub\lightening_plotting\info_storage\GLM_9_7_filtered2.csv"

dataSL = sorter.filter_and_sort_csv(csv_file, "hour", "minute", "second", "millisecond", TIME_FRAME2[0], TIME_FRAME2[1], ascending=True)
dataICandGC = ICandGCHandler.ICandGC(["IC", "GC"], 
...
timeSL = np.array(hSL*3600+60*mSL+(sSL+msSL))
...
def on_click(event):
    if event.inaxes is not None:
        ...
        if distancesC[indexC] < 0.05 and distancesC[indexC]<=distancesS[indexS]:  
            ...
            text.set_text(f"Clicked on SL ({long[indexS]:.2f}, {lat[indexS]:.2f}) with time {time[indexS]:.2f} and current {Current[indexS]:.2f}")  # Update text
            fig.canvas.draw_idle()
        else:
